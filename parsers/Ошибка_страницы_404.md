...
Скрипт перебирает все кампании и группы. В группе выбирается первое активное объявление, из которого парсим final url.
Скрипт
...
function main(){
  ...
  var camp_it = AdWordsApp.campaigns().get(); //Можно сузить перебор кампаний добавиб блок withCondition(). Блоков может быть несколько
